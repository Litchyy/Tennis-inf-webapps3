...
if(isset($_SESSION['email'])){
    ...
    $sql = mysqli_query($mysql,"SELECT l.voornaam, l.naam, l.lidnr, a.accountid, a.email FROM accounts a LEFT JOIN leden l ON a.lidnr = l.lidnr where username= '$usermail' or email = '$usermail'") or die("De selectquery op de database is mislukt!");
    if (mysqli_num_rows($sql) == 1) {
        $row = mysqli_fetch_row($sql);
        $_SESSION['voornaam'] = $row[0];
        $_SESSION['achternaam'] = $row[1];
        $_SESSION['lidnr'] = $row[2];
        $_SESSION['accountid'] = $row[3];
...
<html>
	<head>
		...
		<title><?php echo"".$_SESSION['voornaam']." &#8211 D Tennis"; ?></title>
		...
		<style id='theme-styles-inline-css' type='text/css'>
			...
			.mk-header-bg { background:-webkit-linear-gradient(left,purple 0%, #4B91F1 100%);background:linear-gradient(to left,purple 0%, #4B91F1 100%) } 
			.mk-classic-nav-bg { background:-webkit-linear-gradient(left,purple 0%, #4B91F1 100%);background:linear-gradient(to left,purple 0%, #4B91F1 100%) } 
			...
			<div id="mk-theme-container" >
				<header data-height='160'
					...
					<section id="mk-page-introduce" class="intro-left">
						<div class="mk-grid">
							<h1 class="page-title" style=text-align:center>Account</h1>
							<div id="mk-breadcrumbs">
								<div class="mk-breadcrumbs-inner"><span xmlns:v="http://rdf.data-vocabulary.org/#"><span typeof="v:Breadcrumb"><a href="https://v21dkoeve.helenparkhurst.net/Informatica/.Tennis%20PO/index.php" rel="v:url" property="v:title">Home</a> &#47; <span rel="v:child" typeof="v:Breadcrumb">Account</span></span></span></div>
							</div>
							<div class="clearboth"></div>
						</div>
					</section>
                ...
                <div data-role="header">
						<h1 style="margin-left:20%"><?php $current_time = date('H'); if ($current_time >= 5 && $current_time < 12) { $timeoftheday = "Goedemorgen"; } elseif ($current_time >= 12 && $current_time < 18) { $timeoftheday = "Goedemiddag"; } 
                        else { $timeoftheday = "Goedenavond"; } echo"$timeoftheday ".$_SESSION['voornaam']." ".$_SESSION['achternaam']."!"; ?></h1>
				</div>
                <div class="ui-body ui-corner-all" style="margin-left:19.5%;width:56%;font-size:20px;">
					<?php
                        $mysql = mysqli_connect($server,$user,$pass,$db) or die("Fout: Er is geen verbinding met de MySQL-server tot stand gebracht!");
                        $lidnr = $_SESSION['lidnr'];
                        ...
                        echo"<strong style=font-size:27px;margin-left:2%>Jouw gegevens:<br></strong>";
                        while(list($lidnr, $voornaam, $dbachternaam, $adres, $woonplaats, $telefoonnr, $geslacht, $geboortedatum, $inschrijfdatum, $enkel, $dubbel, $gewonnen, $verloren, $boetenr, $boetedatum, $boetebedrag) = mysqli_fetch_row($boetes))
                        {
                            ...
                            echo"<div style='margin-left:3%'><strong>AccountID:</strong> ".$_SESSION['accountid']."<br><strong>Lidnr:</strong> $lidnr <br><strong>Naam:</strong> $naam <br><strong>Voornaam:</strong> $voornaam <br> 
                            <strong>Achternaam:</strong> $dbachternaam <br> <strong>Geboortedatum:</strong> $geboortedatumm<br> 
                            <strong>Leeftijd:</strong> $age jaar<br><strong>Adres:</strong> $adres <br><strong>Woonplaats:</strong> $woonplaats<br>
                            <strong>Telefoonnummer:</strong> <label style='text-decoration: underline;color:#4B91F1'><a href='tel:$telefoonnr'>$telefoonnr</a></label><br>
                            <strong>E-mailadres:</strong> <label style='text-decoration: underline;color:#4B91F1'><a href='mailto:".$_SESSION['email']."'>".$_SESSION['email']."</a></label><br><strong>Geslacht:</strong> $geslacht<br> 
                            <strong>Enkel:</strong> $enkel<br> <strong>Dubbel:</strong> $dubbel<br><strong>Contributiekosten (Per Seizoen):</strong> €$contributie<br><br></div>
                            <div style='font-size: 27px;margin-left:2%'><span class='tel-font'><strong>Wedstrijdstatistieken:</strong></span></div><div style='margin-left:3%'>
                            <strong>Gewonnen:</strong> " . ($gewonnen ? $gewonnen : "N/A") . "<br> <strong>Verloren:</strong> " . ($verloren ? $verloren : "N/A") . " <br><br>
                            </div><label style='font-size:27px;margin-left:2%'><strong>Boetes:</strong></label><br><div style='margin-left:3%'><strong>Boetenr:</strong> " . ($boetenr ? $boetenr : "N/A") . "<br> 
                            <strong>Boetedatum:</strong> " . ($boetedatum ? $boetedatum : "N/A") . "<br>
                            <strong>Boetebedrag:</strong> " . ($boetebedrag ? $boetebedrag : "N/A") . "</div>";
                        }
				...
				<div id="pag1" data-role="page" data-theme="b">
                    <div class="centerform">
                        <form action="logout.php" method="POST" >
                            <input style="background:-webkit-linear-gradient(left,purple 0%, #4B91F1 100%);background:linear-gradient(to left,purple 0%, #4B91F1 100%)" id="register" type="submit" name="submit" value="Log uit" class="centerform">
                        </form>
                    </div>
				</div>
				<!-- einde body input-->
